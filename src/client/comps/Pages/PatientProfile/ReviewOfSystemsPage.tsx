...
async function getReviewOfSystems(patientID: number) {
  const res = await fetch(`/api/reviewOfSystems/${patientID}`, {
    method: "GET",
  });
  return await res.json();
}

async function postReviewOfSystemsInfo(
  patientID: number,
  data: ReviewOfSystemsState
) {
  const spec = {
    method: "POST",
    headers: {
...
export const ReviewOfSystemsPage: IndividualPatientProfile = ({
  ...
  function checkDependency(category: category, symptomName: string): boolean {
    return (
      ...
      dependencies[symptomName].reduce(
        (acc: boolean, elem: string) => acc && !category[elem],
        true
      )
    );
  }
    ...
    <>
      ...
      >
        <div
          className={
            isShowingSidebar
              ? "review-of-systems-page-outermost-container patient-profile-window"
              ...
              return (
                <div key={category}>
                  ...
                  {Object.keys(state[category]).map((symptomName: string) => {
                    ...
                    if (
                      !(symptomName in genderRelatedSymptoms) ||
                      (symptomName in genderRelatedSymptoms &&
                        genderRelatedSymptoms[symptomName] === gender)
                    ) {
